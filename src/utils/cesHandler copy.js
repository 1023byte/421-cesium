...
export function drawStart() {
  ...
  handler.setInputAction((click) => {
    if (isDraw) {
      ...
      // //点击到图形上,排除动态顶点
      // const isEntity =
      //   pickedObject?.id instanceof Cesium.Entity && !pickedObject?.id.isFloating;
      ...
      if (Cesium.defined(position) && !isEntity) {
        ...
        if (!tempShape) {
          ...
          //绘制动态多边形
          const DynamicPolygon = drawDynamicPolygon(tempPoints);
          const shape = drawPolygon(DynamicPolygon);
          tempShape = shape.id;
          ShapePoints[shape.id] = {};
          ShapePoints[shape.id].coordinates = tempPoints;
